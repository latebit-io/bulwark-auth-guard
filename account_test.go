...
func TestAccountCreateAndVerify(t *testing.T) {
	...
	authenticated, err := guard.Authenticate.Password(ctx, email, deviceID, "password12!P")
	if err != nil {
		t.Error(err)
	}
	...
	authenticated, err = guard.Authenticate.Password(ctx, email, deviceID, "newPassword12!P")
	if err != nil {
		t.Error(err)
	}
